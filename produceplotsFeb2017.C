...
void MakeCorrectionEfficiencyCorrPt(const Char_t * inFileData, const Char_t * inFileEff, const Int_t i_so, const Char_t * outDir){
        ...
        Double_t pStop = 10.0;
        Double_t ptintfit=2.0;

        const Int_t binStart = hEff->GetXaxis()->FindBin(pStart+0.01);  //0.01 
        ...
        Double_t chisptf2sndf[nbinesntrk-initbin+1];
        Double_t chispl4sndf[nbinesntrk-initbin+1];
	...
	funo->SetLineStyle(2);
 	funo->SetLineWidth(2);
        ...
        TLegend *leg23_1[nbinesntrk-initbin+1]; 
         TPad* pRaaPions[3][nbinesntrk-initbin+1];
...
//			Printf("frac");
			...
			else{hPtIn[i_ntrk-initbin]->SetBinContent(bin,0);
                             hPtIn[i_ntrk-initbin]->SetBinError(bin,0);
			}      
...
// /*
                ...
                ptfunction2[i_ntrk-initbin]->SetParameter(0,190);//80);
		ptfunction2[i_ntrk-initbin]->SetParameter(1,4.1);// 4.9);
		ptfunction2[i_ntrk-initbin]->SetParameter(2,0.62);//0.71);
		ptfunction2[i_ntrk-initbin]->SetLineWidth(2);
                powerlaw4[i_ntrk-initbin]->SetParameter(0,11.2);//54.931915);
                powerlaw4[i_ntrk-initbin]->SetParameter(1,-4.5);// -5.2);

                hPtIn[i_ntrk-initbin]->Fit(Form("ptfunction2_%d",i_ntrk),"MN","",pStart,ptintfit);
                hPtIn[i_ntrk-initbin]->Fit(Form("powerlaw4_%d",i_ntrk),"MN2","",ptintfit,pStop);
                ...
                chisptf2sndf[nbinesntrk-initbin]=chisptf2[i_ntrk-initbin]/25.0;//;ndfptf2[i_ntrk-initbin];
                //if(ndfpl4[i_ntrk-initbin]>0){
/*
                if(i_ntrk<80){
                //if(chispl4[i_ntrk-initbin]>0.01){
                  chispl4sndf[nbinesntrk-initbin]=chispl4[i_ntrk-initbin];
                 }else chispl4sndf[nbinesntrk-initbin]=0;
 */
                 if(isnan(chispl4[i_ntrk-initbin])){
                        Printf("is NAN----------------------------------------------------------------->");
			chispl4sndf[nbinesntrk-initbin]=0;
 		}
                 //  else chispl4sndf[nbinesntrk-initbin]=chispl4[i_ntrk-initbin];
                 //chispl4sndf[nbinesntrk-initbin]=chispl4[i_ntrk-initbin]; // /7.0;//;//ndfpl4[i_ntrk-initbin];
	         	
                //}else chispl4sndf[nbinesntrk-initbin]=0;
		cout<<"chi2ptf2="<<chisptf2[i_ntrk-initbin]<<"ndfptf2="<<ndfptf2[i_ntrk-initbin]<<endl;
                cout<<"chi2pl4="<<chispl4[i_ntrk-initbin]<<"ndfpl4="<<ndfpl4[i_ntrk-initbin]<<endl;
                hChi2->SetBinContent(i_ntrk-initbin,chisptf2sndf[nbinesntrk-initbin]);
                hChi2pl->SetBinContent(i_ntrk-initbin,chispl4[i_ntrk-initbin]/7.0);
      //          hChi2pl->SetBinContent(i_ntrk-initbin,chispl4[i_ntrk-initbin]/ndfpl4[i_ntrk-initbin]);
                //hChi2pl->SetBinContent(i_ntrk-initbin,chispl4sndf[i_ntrk-initbin]);
		...
		Double_t totalpt2 = 0;
                ...
                for(Int_t bin = binStart; bin <= binStop; ++bin){ 
                 ...
                 //      cout<<"  low edge, pt="<<hPtIn[i_ntrk-initbin]->GetBinLowEdge(bin)<<"pt="<<pt2<<endl;
                      ...
                      if(pt2>=ptintfit)  yield2 = powerlaw4[i_ntrk-initbin]->Eval(pt2);
                      ...
                      Double_t deta = 1.6;
                        ...
                        hPtInext[i_ntrk-initbin]->SetBinContent(bin, 0);
                        hPtInext[i_ntrk-initbin]->SetBinError(bin, 0);
                        }
...
//  TPad* pRaaPions[nPadX*nPadY][i_ntrk-initbin];
  Float_t shift = 0.05;
  const char* yTitleRaaPions = "data/extrapol  d^{2}#it{N}_{ch} / (N_{ev} 2#pi#it{p}_{T}d#etad#it{p}_{T}) (GeV/#it{c})^{-2}";
...
///*
    ...
    hframe[index][i_ntrk-initbin]->GetXaxis()->SetRangeUser(0.15,10);
	...
	hPtIn[i_ntrk-initbin]->SetMarkerStyle(24);
        ...
        hPtInext[i_ntrk-initbin]->SetMarkerStyle(29); 
        ...
        powerlaw4[i_ntrk-initbin]->SetLineColor(kBlack);
        ...
        leg23_1[i_ntrk-initbin] = new TLegend(0.2,0.1,0.6,0.6);
        ...
        leg23_1[i_ntrk-initbin]->AddEntry(powerlaw4[i_ntrk-initbin],"power law fit, p_{T} > 2 GeV/c","L");
...
TH1D *hframe2[2];
  for(Int_t j=0;j<2;++j)
    hframe2[j] = new TH1D(Form("hframe2_%d",j),Form("hframe2_%d",j),50,0,100);
  gStyle->SetPalette(1);
  gStyle->SetCanvasColor(10);
  gStyle->SetFrameFillColor(10);
  gStyle->SetOptStat(0);
  ...
  Float_t width       = (1-rightMargin-leftMargin)/nPadX;
  Float_t height      = (1-botMargin-topMargin)/nPadY;
  TCanvas* cRaaPionsAll2 = new TCanvas("closureTest2", "Raa Pions2", 550, 800);
  cRaaPionsAll2->SetFillStyle(0);
  TPad* pRaaPions2[nPadX*nPadY];
  Float_t shift = 0.05;
  const char* yTitleRaaPions2 = Form(" data/extrapol    #LT #it{p}_{T} #GT (GeV/#it{c}), (|#eta|<%1.1f)", eta_for_pt);
  const char* xTitleRaaPions2 = Form("N_{ch} (N_{trk}), (|#eta|<%1.1f)", eta_for_ntrk);
  cRaaPionsAll2->cd();
  ...
  latex->SetTextAngle(0);
  latex->SetTextFont(42);
  latex->DrawLatex(0.5,0.04,xTitleRaaPions2);
  latex->SetTextAngle(90);
  latex->SetTextSize(0.055);
  latex->DrawLatex(0.05,0.52,yTitleRaaPions2);
  latex->SetTextSize(0.07);
  latex->SetTextAngle(270);
  latex->SetTextSize(0.04);
  ...
  for (Int_t i = 1; i < 3; i++) {
   ...
   Float_t x2 = leftMargin + (iX + 1)*width;
     ...
     Float_t y1 = 1 - topMargin - (iY +1)*height;
     Float_t y2 = 1 - topMargin - iY*height;
     pRaaPions2[i-1] = new TPad(Form("padRaapions2%d",i),"", x1, y1, x2, y2, 0, 0, 0);
     Float_t mBot = noMargin;
     Float_t mTop = noMargin;
     Float_t mLeft = noMargin;
     Float_t mRight = noMargin;
     ...
     if(iX==0)       mLeft  = 0.08;
     if(iX==nPadX-1) mRight = 0.08;
     pRaaPions2[i-1]->SetBottomMargin(mBot);
     pRaaPions2[i-1]->SetTopMargin(mTop);
     pRaaPions2[i-1]->SetLeftMargin(mLeft);
     pRaaPions2[i-1]->SetRightMargin(mRight);
     pRaaPions2[i-1]->SetFillStyle(0);
     pRaaPions2[i-1]->SetLeftMargin(0.13);
     if(i==2)
       pRaaPions2[i-1]->SetBottomMargin(0.08);
     cRaaPionsAll2->cd();
     pRaaPions2[i-1]->Draw();
  ...
  for (Int_t index = 1; index < 3; index++) {
    ...
    pRaaPions2[index-1]->cd();
    pRaaPions2[index-1]->SetTickx(1);
    pRaaPions2[index-1]->SetTicky(1);
    pRaaPions2[index-1]->SetLogy(0);
    if(index==2) pRaaPions2[index-1]->SetGridy();
    hframe2[index-1]->GetYaxis()->SetLabelSize(0.076);
    hframe2[index-1]->GetXaxis()->SetTitle("");
    hframe2[index-1]->GetYaxis()->SetTitle("");
    hframe2[index-1]->SetLineColor(0);
    if(index == 1){
      Printf("if");
      hframe2[index-1]->Draw();
      hframe2[index-1]->GetYaxis()->SetRangeUser(0.4,1.2);
      hPtvsNm->SetMarkerSize(1.0);
      hPtvsNmext->SetMarkerSize(1.0);
      hPtvsNm->SetMarkerStyle(20);
      hPtvsNmext->SetMarkerStyle(24);
      hPtvsNm->SetMarkerColor(kBlue);
      hPtvsNmext->SetMarkerColor(kRed);      
      hPtvsNm->Draw("samep");
      hPtvsNmext->Draw("samep");
      TLegend *legA = 0;
      legA = new TLegend(0.26,0.55,0.93,0.94);
      legA->SetTextFont(42);
      legA->SetTextSize(0.052);
      legA->SetLineColor(kWhite);
      legA->SetLineWidth(0);
      legA->SetLineStyle(3);
      legA->SetShadowColor(kWhite);
      legA->SetFillColor(kWhite);
      legA->SetFillStyle(0);
      legA->SetHeader("#LT #it{p}_{T} #GT");
      legA->AddEntry(hPtvsNm, Form("data %s",endName));
      legA->AddEntry(hPtvsNmext,"result with extrapolation");
   legA->Draw();
    }
    else{
      Printf("else");
      hframe2[index-1]->GetYaxis()->SetRangeUser(0.95,1.05);
      hframe2[index-1]->GetXaxis()->SetLabelSize(0.08);
      hframe2[index-1]->Draw();
      funo->Draw("same");
     hdivdatatoextmpt->SetLineWidth(2);
     hdivdatatoextmpt->SetMarkerStyle(34);
     hdivdatatoextmpt->Draw("samep");
    }

  }
   cRaaPionsAll2->SaveAs(Form("%s/%s/%s.png",outDir,endName,cRaaPionsAll2->GetName()));

///*
        TCanvas *cBB = 0;
        cBB = new TCanvas("Chi2", "Alice Figure Template", 600, 600);
        cBB->SetLeftMargin(0.15);
        cBB->SetLogy();
        TH1 * hBB = 0;
        hBB = cBB->DrawFrame(0,1e-6,100, 2000000);
        hBB->SetXTitle(Form("#it{N}_{trk}, (|#eta|<%1.1f)", eta_for_ntrk));
        hBB->SetYTitle(Form("#Chi^{2}_{ndf}, |#eta|<%1.1f", eta_for_pt));
        hBB->GetYaxis()->SetTitleOffset(1.5);
        hBB->GetXaxis()->SetTitleOffset(1.05);
       // hChi2->SetMarkerSize(1.0);
       // hChi2->SetMarkerStyle(20);
        hChi2->SetLineColor(kBlue);
       // hChi2pl->SetMarkerSize(1.0);
       // hChi2pl->SetMarkerStyle(24);
        hChi2pl->SetLineColor(kBlack);
       
      TLegend* legCq = new TLegend(0.26,0.55,0.93,0.94);
      legCq->SetTextFont(42);
      legCq->SetTextSize(0.052);
      legCq->SetLineColor(kWhite);
      legCq->SetLineWidth(0);
      legCq->SetLineStyle(3);
      legCq->SetShadowColor(kWhite);
      legCq->SetFillColor(kWhite);
      legCq->SetFillStyle(0);
      legCq->SetHeader("fitting function");
      legCq->AddEntry(hChi2, "Hagerdon fit","l");
      legCq->AddEntry(hChi2pl,"Power law fit","l");
      legCq->Draw();
        hChi2->Draw("same");
        hChi2pl->Draw("same");
